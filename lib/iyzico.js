...
export function createIyzicoClient() {
 ...
 if (!apiKey || !secretKey || apiKey === 'your_api_key_here' || secretKey === 'your_secret_key_here') {
  if (process.env.NODE_ENV === 'production') {
   throw new Error('Ödeme sistemi şu anda kullanılamıyor. Lütfen daha sonra tekrar deneyin veya bizimle iletişime geçin.');
  } else {
   throw new Error('İyzico API anahtarları yapılandırılmamış. Lütfen .env içindeki IYZICO_API_KEY ve IYZICO_SECRET_KEY değerlerini kontrol edin.');
  }
 }
