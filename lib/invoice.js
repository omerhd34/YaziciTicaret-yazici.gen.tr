...
export function canShowInvoice(status) {
...
export function buildInvoiceHtml({ order, userName }) {
 ...
 const rows = items.map((it, i) => {
  ...
  const productName = it.name || it.productName || it.title || 'Ürün';
  return `
      <tr class="${rowClass}">
        <td class="cell-product">${escapeHtml(productName)}</td>
