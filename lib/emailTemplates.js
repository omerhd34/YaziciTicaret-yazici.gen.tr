...
export const orderStatusUpdateTemplate = ({
 ...
 const safeMessage = String(statusMessage || "").replace(/</g, "&lt;").replace(/>/g, "&gt;");

 const content = `
  ${emailTitle("Sipariş Durumunuz Güncellendi.")}
  ${emailParagraph(`Merhaba ${userName || "Müşterimiz"},`)}
  <p style="margin: 0 0 35px 0; color: #4a5568; font-size: 16px; line-height: 1.6; word-wrap: break-word; overflow-wrap: break-word; word-break: break-word; max-width: 100%;">${safeMessage}</p>
...
// Müşteri sipariş alındı (ödeme bekleniyor) şablonu — sipariş oluşturulduğunda gönderilir
export const userOrderReceivedTemplate = ({
 userName,
 orderId,
 orderDate,
 total,
 ...
 const content = `
  ${emailTitle("Siparişiniz Alınmıştır")}
  ${emailParagraph(`Merhaba ${userName || "Müşterimiz"},`)}
  ${emailParagraph("Siparişiniz başarıyla alınmıştır. Ödeme işlemini tamamlamanız için güvenli ödeme sayfasına yönlendirileceksiniz. Ödemeniz onaylandığında siparişiniz kesinleşecektir.", "35px")}
  ${emailInfoBox(`
   <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
     <tr>
       <td style="padding-bottom: 20px; border-bottom: 1px solid #e2e8f0;">
...
// Müşteri sipariş faturası e-postası (sipariş verildiğinde ve Kargoya Verildi olunca)
export const userInvoiceTemplate = ({
 ...
 const content = `
  ${emailTitle("Sipariş Faturanız")}
  ${emailParagraph(`Merhaba ${userName || "Müşterimiz"},`)}
  ${emailParagraph("Siparişinize ait fatura bilgileri aşağıdadır. Bu e-postayı kayıt olarak saklayabilirsiniz.", "35px")}
  ${emailInfoBox(`
   <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
     <tr>
       <td style="padding-bottom: 20px; border-bottom: 1px solid #e2e8f0;">
         ${emailLabelValue("SİPARİŞ NO", orderId || "-")}
       </td>
     </tr>
     <tr>
       <td style="padding: 20px 0; border-bottom: 1px solid #e2e8f0;">
  ...
  `)}
  ${addressSummary ? `${emailSectionTitle("Teslimat Adresi")}${emailAddressBox(addressSummary)}` : ""}
  ${emailSectionTitle("Fatura Kalemleri")}
  ${emailItemsList(items)}
  ${emailButton("Faturayı Görüntüle", `${baseUrl}/hesabim?tab=siparisler`)}
 `;

 return emailWrapper(
  `${emailHeader({ title: "Sipariş Faturası", orderId })}
   ${emailContent(content)}
   ${emailFooter()}`,
  "Sipariş Faturası - Yazıcı Ticaret"
 );
};
...
export const adminReturnRequestTemplate = ({
 ...
 const imageBlock = urls.length > 0
  ? `
  <div style="margin: 16px 0;">
   <div style="font-size: 12px; color: #6b7280; margin-bottom: 8px; font-weight: 600;">MÜŞTERİ TARAFINDAN EKLENEN RESİMLER</div>
   ${urls.map((u, i) => `<a href="${u}" target="_blank" style="display: inline-block; margin: 4px;"><img src="${u}" alt="İade resmi ${i + 1}" style="max-width: 200px; max-height: 200px; border-radius: 8px; border: 1px solid #e5e7eb;" /></a>`).join("")}
  </div>`
  : "";
...
export const priceChangeTemplate = ({
 ...
 productUrl,
 baseUrl,
 source = 'favorite', // 'favorite' veya 'cart'
}) => {
 ...
 const title = source === 'cart' ? 'Sepetteki Ürününüzde Fiyat Değişikliği' : 'Favori Ürününüzde Fiyat Değişikliği';
 ...
 const content = `
  ...
  ${emailParagraph(`${locationText} <strong style="color: #2d3748;">"${productName}"</strong> ürününün fiyatı <strong style="color: ${priceColor};">${priceChange}</strong>!`, "35px")}
  ${emailInfoBox(`
   <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
     <tr>
...
export const stockAvailableTemplate = ({ userName, productName, productUrl, baseUrl, source = 'favorite' }) => {
 // Başlık ve mesajı source'a göre değiştir
 const title = source === 'cart' ? 'Sepetteki Ürününüz Stokta!' : 'Favori Ürününüz Stokta!';
 const locationText = source === 'cart' ? 'Sepetinizdeki' : 'Favorilerinizdeki';

 const content = `
  ${emailTitle(title)}
  ${emailParagraph(`Merhaba <strong style="color: #2d3748;">${userName}</strong>,`)}
  ${emailParagraph(`${locationText} <strong style="color: #2d3748;">"${productName}"</strong> ürünü artık stokta!`, "35px")}
  ${emailButton("Ürünü Görüntüle", productUrl)}
  ${emailNotice(`
   Bu bildirimi almak istemiyorsanız, <a href="${baseUrl}/hesabim" style="color: #4299e1; text-decoration: underline;">hesap ayarlarınızdan</a> e-posta bildirimlerini kapatabilirsiniz.
  `)}
