...
// Müşteriye sipariş faturası e-postası gönder (sipariş onayında ve Kargoya Verildi durumunda)
export const sendUserInvoiceEmail = async ({
 userEmail,
 userName,
 orderId,
 orderDate,
 ...
 try {
  ...
  const mailOptions = {
   from: `Yazıcı Ticaret <${process.env.EMAIL_USER}>`,
   to: userEmail,
   subject: `📄 Sipariş Faturası - ${orderId}`,
   html: userInvoiceTemplate({
    userName: safeName,
    orderId,
    orderDate,
    total,
   ...
   text: `
     ...
     Siparişinize ait fatura bilgileri.
     ...
     Faturayı görüntülemek için: ${baseUrl}/hesabim?tab=siparisler
